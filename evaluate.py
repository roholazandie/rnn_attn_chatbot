...
def evaluate1(encoder, decoder, searcher, vocab, sentence, max_length=MAX_LENGTH):
    # word -> indexes
    indexes_batch = [vocab.string2ids(sentence) + [vocab.eos_id]]
    # create length tensor
    lengths = torch.tensor([len(indexes) for indexes in indexes_batch])
    # transpose dimensions of batch to match model's expectations
    input_batch = torch.LongTensor(indexes_batch).transpose(0, 1)
    ...
    tokens, scores = searcher(input_batch, lengths, max_length)
    # indexes -> words
    decoded_words = vocab.ids2string([token.item() for token in tokens])
    return decoded_words
...
def evaluate_input1(encoder, decoder, searcher, vocab):
    input_sentence = ""
    while True:
        try:
            # get the input
