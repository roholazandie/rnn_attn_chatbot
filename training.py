...
def train(config, vocab, input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding,
    ...
    if use_teacher_forcing:
        ...
        for t in range(max_target_len):
            ...
            decoder_input = torch.LongTensor([topi[i][0] for i in range(config.batch_size)]).unsqueeze(0).to(config.device)
            outputs[t] = decoder_outputs
    ...
    cross_entropy = -torch.log(
        torch.gather(outputs.view(-1, outputs.shape[-1]), 1, target_variable.view(-1, 1)).squeeze(1)).reshape(-1,
                                                                                                              config.batch_size)
    loss = cross_entropy.masked_select(mask).mean()
    loss.backward()
...
def train_orig(config, vocab, input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder,
               embedding,
               encoder_optimizer, decoder_optimizer):
    # zeros gradients
    encoder_optimizer.zero_grad()
    ...
    for iteration, training_batch in enumerate(data_loader):
        ...
        try:
            ...
            torch.save({"iteration": iteration,
                        ...
                        "voc_dict": voc.__dict__,
                        "embedding": embedding.state_dict()},
                       os.path.join(directory, 'interrupted_checkpoint.tar'))